...
// Global flag that enables the new Elm�Soft protocol for maximum USB throughput (Candlelight only).
// It is not possible to enable the Elm�Soft protocol for an individual channel, 
// because Elm�Soft uses different USB interfaces while Legacy routes all traffic through the first interface.
...
void              buf_process_can  (int channel, buf_class* can_buf);
...
bool              buf_store_can_frame(int channel, uint8_t* can_frame);
void              buf_store_rx_packet_echo(int channel, FDCAN_RxHeaderTypeDef *rx_header, uint8_t *rx_data, uint32_t fake_echo);
...
void buf_process_can(int channel, buf_class* can_buf)
{
    ...
    if (GLB_ProtoElmue) // new Elm�Soft protocol
    {
        // The new Elm�Soft firmware sends an echo marker when the packet has REALLY been dispatched to CAN bus.
...
void buf_store_can_frame_blob(int channel, uint8_t* can_frame)
{
...
bool buf_store_can_frame(int channel, uint8_t* can_frame)
{
    ...
    if (flags & FRM_FDF) // FDF bit is set if recessive
    {
        ...
        // This was totally wrong in the orginal code (fixed by Elm�soft)
...
bool buf_store_tx_packet(int channel, FDCAN_TxHeaderTypeDef* tx_header, uint8_t* tx_data)
{
    buf_class* can_buf = &buf_inst[channel];
...
{
    if (GLB_ProtoElmue)     
        return &buf_inst[channel]; // Elm�Soft -> send each CAN channel through it's own USB interface 0, 2 or 3
    else
